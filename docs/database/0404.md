...
索引的本质=用额外 2～3 倍的存储空间，换查询 100～1000 倍的速度，B+ 树通过【树高只有 3～4 层（log n 深度）+ 叶子链表范围查询 + 页对齐 + 内部节点只存指针不存数据】四个设计，把百万行级别的查找从“读百万字节”压缩到“读几十字节 +3～4 次随机读页”
